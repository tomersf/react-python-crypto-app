...
def test_is_valid_chain_bad_genesis(blockchain_five_blocks):
    blockchain_five_blocks.chain[0].hash = 'bad_hash'
...
def test_replace_chain_not_longer(blockchain_five_blocks):
    ...
    with pytest.raises(Exception, match='Cannot replace chain, must be longer!'):
        blockchain_five_blocks.replace_chain(blockchain.chain)


def test_repalce_chain_bad_chain(blockchain_five_blocks):
    blockchain = Blockchain()
    blockchain_five_blocks.chain[1].hash = 'bad_hash'
...
def test_is_valid_transaction_chain_duplicate_transactions(blockchain_five_blocks):
