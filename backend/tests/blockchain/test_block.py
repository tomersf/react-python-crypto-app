...
def test_is_valid_block_difficulty(last_block, block):
    ...
    with pytest.raises(Exception, match='Block difficulty must only adjust by 1 from prev block'):
        Block.is_valid_block(last_block, block)


def test_is_valid_block_bad_block_hash(last_block, block):
    block.hash = '000000000000aaaabbbcccdd'

    with pytest.raises(Exception, match='The block hash is corrupted!'):
        Block.is_valid_block(last_block, block)
